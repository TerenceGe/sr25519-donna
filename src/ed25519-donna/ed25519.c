...
static void
ed25519ph_hram(hash_512bits hram, const ed25519_signature RS, const ed25519_public_key pk, const unsigned char *dom2, size_t dom2_len, const unsigned char *m, size_t mlen) {
	ed25519_hash_context ctx;
	ed25519_hash_init(&ctx);
	ed25519_hash_update(&ctx, dom2, dom2_len);
	ed25519_hash_update(&ctx, RS, 32);
	ed25519_hash_update(&ctx, pk, 32);
	ed25519_hash_update(&ctx, m, mlen);
	ed25519_hash_final(&ctx, hram);
...
void
ED25519_FN(ed25519ph_sign) (const unsigned char *m, size_t mlen, const ed25519_secret_key sk, const ed25519_public_key pk, const unsigned char *dom2, size_t dom2_len, ed25519_signature RS) {
	ed25519_hash_context ctx;
	bignum256modm r = {0}, S = {0}, a = {0};
	ge25519 ALIGN(16) R = {0};
	hash_512bits extsk = {0}, hashr = {0}, hram = {0};
	...
	/* S = H(R,A,m).. */
	ed25519ph_hram(hram, RS, pk, dom2, dom2_len, m, mlen);
	expand256_modm(S, hram, 64);
...
int
ED25519_FN(ed25519ph_sign_open) (const unsigned char *m, size_t mlen, const ed25519_public_key pk, const unsigned char *dom2, size_t dom2_len, const ed25519_signature RS) {
	ge25519 ALIGN(16) R, A;
	hash_512bits hash = {0};
	bignum256modm hram = {0}, S = {0};
	unsigned char checkR[32] = {0};
	...
	/* hram = H(R,A,m) */
	ed25519ph_hram(hash, RS, pk, dom2, dom2_len, m, mlen);
	expand256_modm(hram, hash, 64);
